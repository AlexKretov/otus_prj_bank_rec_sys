...
Документ фиксирует актуальные выводы по последнему обучающему запуску
`modeling.ipynb`. Исходные наблюдения по данным сделаны в `eda.ipynb`,
подробные численные отчёты лежат в `artifacts/`.
...
Что из EDA влияет на моделирование:

- `conyuemp` и `ult_fec_cli_1t` почти пустые, `renta` — около 20% пропусков
  и тяжёлый правый хвост;
- аномалии в числовых признаках: возраст до 164 лет, экстремальные доходы,
  некорректные значения стажа;
- клиентская база сконцентрирована (99.6% — Испания; Madrid и Barcelona;
  каналы `KHE`, `KAT`, `KFC`; около 57.8% клиентов неактивны);
- продуктовое потребление неравномерно: текущий счёт доминирует, большинство
  продуктов редкие — отсюда сильный дисбаланс таргета и выбор метрик.

## 2. Предобработка

Обучение и сервис используют один модуль `preprocessing.py`. Пайплайн:
